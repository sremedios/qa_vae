...
def show_image(img_data):
    plt.imshow((img_data*255).astype(np.uint8), interpolation='nearest', cmap="Greys")
    plt.show()

def compare_histograms(img_data_1, img_data_2, test_loc, ignore_zero=True):

    img_1 = (img_data_1 * 255).T.astype(np.uint8).flatten()
    img_2 = (img_data_2 * 255).T.astype(np.uint8).flatten()
    diff = np.abs((img_data_1 * 255).T - (img_data_2 * 255).T).astype(np.uint8).flatten()
    ...
    kde_orig = KDE(kernel='gaussian', bandwidth=0.75).fit(np.reshape(img_1, img_1.shape + (1,)))
    ...
    kde_recon = KDE(kernel='gaussian', bandwidth=0.75).fit(np.reshape(img_2, img_2.shape + (1,)))
    ...
    orig_recon_hist_inter = histogram_intersection(kde_orig_samples, kde_recon_samples)
